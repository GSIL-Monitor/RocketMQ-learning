...
public class RouteInfoManager {
    ...
    /**
     * topic queue的对应关系的缓存与记录，由于一个topic可以同时存在于多个broker，而每个broker对于该topic来说都有各自的queue，所以
     * 这里使用list来记录queue数据, QueueData中也有字段表示该queue是属于哪一个broker的
     */
    private final HashMap<String/* topic */, List<QueueData>> topicQueueTable;
    /**
     * 在集群模式的部署方式中，Master与Slave配对是通过指定相同的brokerName参数来配对，Master的BrokerId必须是0，
     * Slave的BrokerId必须是大于0的数。一个Master下面可以挂载多个Slave，同一个Master下的多个Slave通过指定不同的BrokerId来区分
     *
     * 该集合就是用来记录同一个broker name下，即master slaver的关系
     */
    private final HashMap<String/* brokerName */, BrokerData> brokerAddrTable;
    /**
     * 多个broker构成一个集群，一个集群中的所有broker共享一个cluster name
     * 该集合记录每个集群下所有的broker信息
     */
    private final HashMap<String/* clusterName */, Set<String/* brokerName */>> clusterAddrTable;
    /**
     * 缓存每个broker最新的状态，用于判断broker信息是否发生了变化
     */
    ...
    public RegisterBrokerResult registerBroker(
        ...
        try {
            try {
                ...
                // 如果topic信息发生变化，那么进行更新name server中信息
...
/**
 * 标记broker的状态，用于判断broker是否发生了更新，以便在name server进行同步更新
 */
