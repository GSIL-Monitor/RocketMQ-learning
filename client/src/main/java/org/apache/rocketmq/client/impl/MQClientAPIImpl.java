...
public class MQClientAPIImpl {
    ...
    /**
     * 是否开启smart message，所谓smart message就是比原有消息使用更短的变量名，从而在fastjson 进行序列化与反序列化时更加快
     * 默认开启
     */
    ...
    /**
     * 四种name server 地址设置的第四种，通过http服务设置
     */
        ...
        this.remotingClient.invokeAsync(addr, request, timeoutMillis, new InvokeCallback() {
            ...
            public void operationComplete(ResponseFuture responseFuture) {
                ...
                if (response != null) {
                    ...
                    // 这里处理发送失败的后处理，更加不同的错误原因，初始化不同的错误环境，然后调用回调函数
                    producer.updateFaultItem(brokerName, System.currentTimeMillis() - responseFuture.getBeginTimestamp(), true);
    ...
    /**
     * 向指定broker发送请求，请求获取该broker上 指定gourp name的所有消费者名称（名称= IP@PID）
     * @param addr
     * @param consumerGroup
     * @param timeoutMillis
     * @return
     ...
     * @param topic
     * @param timeoutMillis
     * @param allowTopicNotExist 是否运行topic不存在
     * @return
     * @throws MQClientException
     * @throws InterruptedException
     * @throws RemotingTimeoutException
    ...
    public TopicRouteData getTopicRouteInfoFromNameServer(final String topic, final long timeoutMillis,
        ...
        // 该方法调用，会真正去从name server获取该topic的路由信息。 addr参数设置为null 表示默认从name server地址获取
        RemotingCommand response = this.remotingClient.invokeSync(null, request, timeoutMillis);
        assert response != null;
        switch (response.getCode()) {
