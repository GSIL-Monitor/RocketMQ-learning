...
/**
 * 名字虽然是 Push 开头，实际在实现时，使用 Pull 方式实现。通过 Pull 不断不断不断轮询 Broker 获取消息。当不存在新消息时，
 * Broker 会挂起请求，直到有新消息产生，取消挂起，返回新消息。这样，基本和 Broker 主动 Push 做到接近的实时性（当然，还是有相应的实时性损失）。
 * 原理类似 长轮询( Long-Polling )。
...
public class DefaultMQPushConsumer extends ClientConfig implements MQPushConsumer {
     ...
     * </p>
     *
     * There are three consuming points: 共有3种消费进度选项
     * <ul>
     * <li>
     ...
     *
     * 全新的消费组定义： 如果这个消费进度在Broker并没有存储起来，证明这个是一个全新的消费组
     *
    ...
    /**
     * 当RocketMQ发现本地缓存的消息的最大值-最小值差距大于这个值（2000）的时候，会触发流控——也就是说如果头尾都卡住了部分消息，达到了这个阈值就不再拉取消息。
     * Concurrently max span offset.it has no effect on sequential consumption
     */
    private int consumeConcurrentlyMaxSpan = 2000;

    // 以下四个变量，用于控制向broker拉取消息的最大数量与数据大小的上限阙值
    /**
