...
/**
 * 封装所有的消息命令，包括请求与响应消息，使用不同标记代表不同的消息类型
 */
public class RemotingCommand {
    ...
    /**
     * 命令头缓存，key：命令头类；value：该类所有的字段
     * 这样做是因为：数据传输时，并不是直接将命令头序列化进行传输，而是将命令头中的所有字段与值放入一个map中，最后对map进行序列化
     */
    ...
    /**
     * 该请求是由哪一版本的rocketmq生成的
     */
    private int version = 0;
    /**
     * 用于记录消息的顺序，并且会在缓冲中使用该变量作为缓存key，并且该值也用来将请求-响应关联起来, 相同该值表示一对
     */
