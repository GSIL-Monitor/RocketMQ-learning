...
public class SendMessageProcessor extends AbstractSendMessageProcessor implements NettyRequestProcessor {
    ...
    public RemotingCommand processRequest(ChannelHandlerContext ctx,
        ...
        switch (request.getCode()) {
            case RequestCode.CONSUMER_SEND_MSG_BACK:
                // 当 Consumer 消费某条消息失败时，会调用该接口发回消息。Broker 会存储发回的消息。这样，
                // 下次 Consumer 拉取该消息，能够从 CommitLog 和 ConsumeQueue 顺序读取
                return this.consumerSendMsgBack(ctx, request);
            default:    // 默认处理生产者发送消息的请求
                // 解析请求头
     ...
     * 下次 Consumer 拉取该消息，能够从 CommitLog 和 ConsumeQueue 顺序读取
     *
     * 还有注意的是：重新消费消息，并不是从原来topic queue上重新获取消息，而是收到发回的消息后，将其当做生产者一样处理，将该消息
     * 放入以消费组名称为基础的 retry topic中，在延迟的某个时间点，再次投递到这个消费组
     *
     * @param ctx
     * @param request
     * @return
    ...
    private RemotingCommand consumerSendMsgBack(final ChannelHandlerContext ctx, final RemotingCommand request)
        ...
        // 获取一条消息最多能够重新消费的最大次数，消费组配置中默认时16
        int maxReconsumeTimes = subscriptionGroupConfig.getRetryMaxTimes();
        if (request.getVersion() >= MQVersion.Version.V3_4_9.ordinal()) {   // 从这里也可以看到，V3.4.9之后，支持消费者端控制次数
        ...
        if (msgExt.getReconsumeTimes() >= maxReconsumeTimes
            ...
            queueIdInt = Math.abs(this.random.nextInt() % 99999999) % DLQ_NUMS_PER_GROUP;   // 加入死信队列后，应用可以之后监控死信队列来做人为干预
     ...
     * @param ctx
     * @param request
     * @param sendMessageContext 发送消息的上下文信息，用于钩子方法
     * @param requestHeader
     * @return
     * @throws RemotingCommandException
     */
    private RemotingCommand sendMessage(final ChannelHandlerContext ctx,
        ...
        // 以下代码将所有消息相关的信息都集成进一个类 MessageExtBrokerInner 中
        MessageExtBrokerInner msgInner = new MessageExtBrokerInner();
     ...
     * @param sendMessageContext 消息发送上下文信息，用于钩子方法
     * @param ctx
     * @param queueIdInt
     * @return
     */
