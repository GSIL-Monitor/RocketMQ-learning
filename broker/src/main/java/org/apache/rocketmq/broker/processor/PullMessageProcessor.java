...
public class PullMessageProcessor implements NettyRequestProcessor {
     ...
     * @param channel
     * @param request 请求
     * @param brokerAllowSuspend 当拉取不到消息时候，是否运行broker 对该请求进行挂起
     * @return
    ...
    private RemotingCommand processRequest(final Channel channel, RemotingCommand request, boolean brokerAllowSuspend)
        ...
        // 校验 consume gourp 是否存在，如果不存在，则自动创建
        ...
        if (hasSubscriptionFlag) {
            ...
            //  检验消费者 组  所有消费者在工作前都会向broker注册，拥有相同的group name的被封装成一个组，所以
            // 每个gourp name都对应一个组信息, 组信息内规定了组内消费者的消费行为，这里对这些消费行为做验证
            ...
            // 验证 如果没有配置中没有开启广播消费，但是组信息却要求这样消费，存在不一致
            ...
            // 如果拉取消息中没有指定订阅消息的方式， 那么使用之前group配置的那种方式
            // 验证 topic的订阅关系是否存在
        ...
        if (getMessageResult != null) {
            ...
            switch (response.getCode()) {
                case ResponseCode.SUCCESS:
                    ...
                    if (this.brokerController.getBrokerConfig().isTransferMsgByHeap()) {    // 是否将消息内容拷贝到堆内存
                        ...
                        try { // 基于 zero-copy 实现，直接响应，无需堆内内存，性能更优
        ...
        storeOffsetEnable = storeOffsetEnable
            && this.brokerController.getMessageStoreConfig().getBrokerRole() != BrokerRole.SLAVE;   // 确保不是slave
        if (storeOffsetEnable) {    // 提交消费进度
    ...
    private byte[] readGetMessageResult(final GetMessageResult getMessageResult, final String group, final String topic,
        ...
        try {
            ...
            getMessageResult.release(); // 释放之前的缓冲区
        }
